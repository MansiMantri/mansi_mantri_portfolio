...
const InternshipCard = ({ intern, index }) => (
  ...
  >
    <div>
      ...
      {/* Skills gained / Responsibilities */}
      <div className="mb-6">
...
const Internships = ({ activeRole }) => {
  const currentProfile = profiles[activeRole] || profiles.software_engineer;
  const internshipsList = currentProfile.internships || [];
      ...
      <div className="max-w-7xl mx-auto relative z-20">
        ...
        <div data-aos="fade-up" className="mb-16 md:mb-20 text-center">
          ...
          <p className="text-red-100 text-base md:text-lg font-semibold max-w-lg mx-auto">
            Practical internships where I applied software engineering and data analytics principles.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 justify-center">
          {internshipsList.map((intern, index) => (
            <InternshipCard key={`${intern.organization}-${intern.role}`} intern={intern} index={index} />
          ))}
        </div>
